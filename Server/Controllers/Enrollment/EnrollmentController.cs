...
namespace SWARM.Server.Controllers.Enrollmentz
{
    ...
    public class EnrollmentController : Controller
    {
        ...
        public async Task<IActionResult> Delete(int pSectionId, int pStudentId)
        {
            ...
            //get every section related, delete those
            //etc etc, delete course
            /*var enrollment = await _context.eEnrollments
                .Include("section")
                .Include("course")
                .Where(x => x.S.Course.CourseNo == pCourseNo).ToListAsync();
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] CourseDTO _Course)
        {
            bool bExist = false;
            var trans = _context.Database.BeginTransaction();
            try
